...
enum GameButton { # as opposed to buttons for navigating menus
	UP,
	DOWN,
	LEFT,
	RIGHT,
...
func is_action_just_pressed(action: GameButton) -> bool:
...
func is_action_pressed(action: GameButton) -> bool:
	var array := _button_states_physics if Engine.is_in_physics_frame() else _button_states
	var state := array[action]
	return state == ButtonState.PRESSED or state == ButtonState.JUST_PRESSED

func is_action_just_released(action: GameButton) -> bool:
	var array := _button_states_physics if Engine.is_in_physics_frame() else _button_states
	return array[action] == ButtonState.JUST_RELEASED

func get_movement_vector() -> Vector2:
	var out := Vector2.ZERO
	out += ((1 if is_action_pressed(GameButton.RIGHT) else 0) - (1 if is_action_pressed(GameButton.LEFT) else 0)) * Vector2(1, 0)
	out += ((1 if is_action_pressed(GameButton.DOWN) else 0) - (1 if is_action_pressed(GameButton.UP) else 0)) * Vector2(0, 1)
...
func _ready() -> void:
	_button_states.resize(GameButton.MAX_BUTTON)
	_button_states.fill(ButtonState.UNPRESSED)
	_button_states_physics.resize(GameButton.MAX_BUTTON)
...
func _remove_just_pressed_or_released(button_state_array: PackedByteArray) -> void:
	for idx in GameButton.MAX_BUTTON:
